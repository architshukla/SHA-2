...
//		wi = sigma1(wi-2)+wi-7+sigma0(wi-15)+wi-16  17<=i<= 64
		for (int k = 16; k < 64; k++) {
			List<Boolean> a = sha.modularAddList(sha.sigma1(W.get(k-2)),W.get(k-7));
			List<Boolean> b = sha.modularAddList(sha.sigma0(W.get(k-15)), W.get(k-16));
			W.add(sha.modularAddList(a, b));
		}
